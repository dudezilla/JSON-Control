...
class ObservedSelectionControl extends Control {
...
ObservedSelectionControl.controlType = 'observed_selection'

ObservedSelectionControl.meta = {
  controlType:   'observed_selection',
  category:      'selection',
  description:   'Like SelectionControl but designed for external observers — value changes are intended to be monitored by spy or logging instrumentation.',
  defaultConfig: { name: 'test_osel', label: 'Observed pick', state: 'X', control_type: 'observed_selection', values: ['X', 'Y', 'Z'] },
  tests: [
    {
      name: 'getValue returns initial selection',
      fn: function (ctrl) { return ctrl.getValue() === 'X' }
    },
    {
      name: 'setValue / getValue roundtrip',
      fn: function (ctrl) { ctrl.setValue('Z'); return ctrl.getValue() === 'Z' }
    },
    {
    ...
    },
    {
      ...
      fn: function (ctrl) { return ctrl.selectValue(1) === 'Y' }
    },
    {
      name: 'onChildEvent updates the value',
      fn: function (ctrl) { ctrl.onChildEvent('Y'); return ctrl.getValue() === 'Y' }
    },
    {
...
export { ObservedSelectionControl }
